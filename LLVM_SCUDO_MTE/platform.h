...
#define SCUDO_RISCV64 1
// [新增] 检查是否启用了 Zimte
// 逻辑：如果编译脚本传了 -DSCUDO_ZIMTE=1，则保持原样；
// 否则检查编译器自带宏 __riscv_zmt__；如果都没有，则默认为 0。
# if !defined(SCUDO_ZIMTE)
#   if defined(__riscv_zmt__) || defined(__RISCV_ZIMTE__)
#     define SCUDO_ZIMTE 1
#   else
#     define SCUDO_ZIMTE 0
#   endif
# endif
#else
#define SCUDO_RISCV64 0
...
#ifndef SCUDO_CAN_USE_MTE
// [修改] 增加 || SCUDO_ZIMTE
// 只要是 Linux/Trusty 或者 显式启用了 Zimte，就允许使用 MTE 逻辑
#define SCUDO_CAN_USE_MTE (SCUDO_LINUX || SCUDO_TRUSTY || SCUDO_ZIMTE)
#endif
